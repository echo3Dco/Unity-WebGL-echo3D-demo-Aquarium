...
/* Adjustments to standard echo3D Unity SDK Script & Libraries:

- disabled WebSocketClient to prevent usage of System.Net classes (System.Net classes are not supported in browser and app does not load)
- disabled GLB stream loading; all assets now downloaded & handled via Siccity.GLTFUtility (Other library does not handle skinned mesh, GLTFUtility handles both regular and skinned mesh)
- disabled adding CustomBehavior.cs script on init (Not used)
- Within Siccity.GLTFUtility: updated Shader.Find() to look for Unity Standard Shader instead of GLTF shader (neither shader is used for a model; fixes exception in live builds)
- Updated DownloadAndInitiateImage() to use current Unity web request error checking
*/
...
public class echo3D : MonoBehaviour
{
    ...
    public bool useStaticModelsProject;
    public Material arch01Material;
    ...
    public Material coralBunch1Material;
    public Material largeShelfCoral4Material;
    public Material trunkWoodMaterial;
    public Material trunkMetalMaterial;
    public Material trunkGoldMaterial;
    ...
    void Awake()
    {
        if (useStaticModelsProject)
        {
            APIKey = Globals.staticModelsProjectApiKey;
            secKey = Globals.staticModelsProjectSecKey;
        }
        else
        {
            APIKey = Globals.fishProjectApiKey;
            secKey = Globals.fishProjectSecKey;
        }
    }
    void Start()
    {
    ...
    IEnumerator QueryDatabase(string serverURL)
    {
        ...
        Debug.Log("Querying database..." + gameObject.name);
    ...
    IEnumerator DownloadAssets(string serverURL)
    {
        ...
        if (GetComponent<FishController>() != null)
        {
        GetComponent<FishController>().manager.assetDownloaded();}
  ...
  //              }
    ...
    IEnumerator InstantiateModel(Entry entry, List<string> filenames, ImportOptions importOptions)
    {
        ...
        //else
         if (extension == ".gltf" || extension == ".glb") {
            ...
            //Debug.Log("GLTF Instantiate");
            GameObject result = shader == null ? Importer.LoadFromFile(filepath) : Importer.LoadFromFile(filepath, shader);
            result.name = filenames[0] + " result";
            result.AddComponent<CustomBehaviour>().entry = entry;
            ...
            result.transform.parent = this.gameObject.transform;
            result.transform.position = GetModelPosition(entryId);
            result.transform.rotation = GetModelRotation(entryId);
            result.transform.localScale = GetModelScale(entryId) == Vector3.zero ? result.transform.localScale : GetModelScale(entryId);
            ModelTweaks(result, entryId);

            //Debug.Log("GLTF Instantiate complete: " + result.name);
            if (gameObject.GetComponent<FishController>() != null) {
            gameObject.GetComponent<FishController>()?.finishedModelLoading();}

            if (GetComponentInChildren<FishController>() !=null){
            GetComponentInChildren<FishController>()?.finishedModelLoading();}
    ...
    //tweaks required for static models (aquarium decor)
    Vector3 GetModelPosition(string entry)
    {
        switch (entry)
        {
            case Globals.chestGold: {
                return new Vector3(26.11f, 3.3f, 5.52f);
            }
            case Globals.chestLid: {
                return new Vector3(27.72f, 3.3f, 5.57f);
            }
            case Globals.chestTrunk: {
                return new Vector3(26.11f, 3.3f, 5.52f);
            }
            case Globals.largeShelfCoral4: {
                return new Vector3(4.5f, 1.1f, 10);
            }
            case Globals.coralBunch3: {
                return new Vector3(4, 0, 33);
            }
            case Globals.coralBunch1: {
                return new Vector3(2.5f, 0, 33);
            }
            case Globals.coral3: {
                return new Vector3(24, 0, 39);
            }
            case Globals.largeShelfCoral2: {
                return new Vector3(10, -0.25f, 41.5f);
            }
            case Globals.blobbyCoralEntryId: {
                return new Vector3(26, 0.5f, 46);
            }
            case Globals.bulbousCoralEntryId: {
                return new Vector3(1f, 0, 8);
            }
            case Globals.smallSoftCoralEntryId: {
                return new Vector3(9,0.5f, 26);
            }
            case Globals.largeSoftCoralEntryId: {
                return new Vector3(14,0,31);
            }
            case Globals.arch01EntryId: {
                return new Vector3(21,0,25);
            }
    ...
    Quaternion GetModelRotation(string entry)
    {
        switch (entry)
        {
            case Globals.arch01EntryId: {
            ...
            case Globals.chestLid:{
                return Quaternion.Euler(2.6f, 89.73f, 82.2f);
            }
            case Globals.chestTrunk:
            case Globals.chestGold:{
                return Quaternion.AngleAxis(-98f, Vector3.right);
            }
            case Globals.bulbousCoralEntryId:
            case Globals.smallSoftCoralEntryId: {
    ...
    Vector3 GetModelScale(string entry)
    {
        switch(entry)
        {
            case Globals.bulbousCoralEntryId:{
            ...
            case Globals.largeSoftCoralEntryId: {
                return new Vector3(0.13f,0.13f,0.13f);
            }
            case Globals.largeShelfCoral4:{
    ...
    void ModelTweaks(GameObject result, string entryID)
    {
        switch(entryID)
        {
            case Globals.largeShelfCoral4:{
                result.GetComponentInChildren<MeshRenderer>().material = largeShelfCoral4Material;
                break;
            }
            case Globals.coralBunch3: {
                result.GetComponentInChildren<MeshRenderer>().material = coralBunch1Material;
                break;
            }
            case Globals.coralBunch1: {
                result.GetComponentInChildren<MeshRenderer>().material = coralBunch1Material;
            ...
            case Globals.coral3: {
                result.GetComponentInChildren<MeshRenderer>().material = blobbyCoralMaterial;
                break;
            }
            case Globals.largeShelfCoral2: {
                result.GetComponentInChildren<MeshRenderer>().material = largeShelfCoral2Material;
                break;
            }
            case Globals.blobbyCoralEntryId: {
                result.GetComponentInChildren<MeshRenderer>().material = blobbyCoralMaterial;
            ...
            case Globals.bulbousCoralEntryId: {
                result.GetComponentInChildren<MeshRenderer>().material = bulbousCoralMaterial;
                break;
            }
            case Globals.smallSoftCoralEntryId: {
                result.GetComponentInChildren<MeshRenderer>().material = smallSoftCoralMaterial;
                break;
            }
            case Globals.arch01EntryId:{
                result.GetComponentInChildren<MeshRenderer>().material = arch01Material;
                break;
            }
            case Globals.largeSoftCoralEntryId: {
                result.GetComponentInChildren<MeshRenderer>().material = largeSoftCoralMaterial;
                result.GetComponentInChildren<MeshRenderer>().material.SetFloat("_FlutterAmount", 0.002f);
                break;
            }
            default: 
            {
