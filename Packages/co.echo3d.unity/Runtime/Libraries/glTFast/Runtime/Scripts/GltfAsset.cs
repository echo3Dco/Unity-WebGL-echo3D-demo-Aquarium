...
namespace GLTFast
{
    ...
    public class GltfAsset : GltfAssetBase
    {
        ...
        //echoedit
        //This tells gltfast whether it should apply model transform data (position, scale, rotation) or leave all zeroed
        public bool zeroTransform = false;
            ...
            )
        {
            ...
            if (success)
            {
                ...
                if (sceneId >= 0)
                {
                    InstantiateScene(sceneId, logger, zeroTransform);
                }
                else
                {
                    Instantiate(logger, zeroTransform);
                }
            }
            return success;
        }
